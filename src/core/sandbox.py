...
DANGER_PATTERNS: List[re.Pattern] = [
    ...
    re.compile(r"\b__import__\s*\(\s*['\"]socket['\"]"),
    re.compile(r"\b__import__\s*\(\s*['\"]ctypes['\"]"),
    re.compile(r"\b__import__\s*\(\s*['\"]shutil['\"]"),
    re.compile(r"\b__import__\s*\(\s*['\"]pickle['\"]"),
    re.compile(r"\b__import__\s*\(\s*['\"]marshal['\"]"),
    re.compile(r"\b__import__\s*\(\s*['\"]pty['\"]"),
    re.compile(r"\b__import__\s*\(\s*['\"]fcntl['\"]"),
    re.compile(r"\b__import__\s*\(\s*['\"]resource['\"]"),
    re.compile(r"\b__import__\s*\(\s*['\"]pwd['\"]"),
    re.compile(r"\b__import__\s*\(\s*['\"]grp['\"]"),
    re.compile(r"\b__import__\s*\(\s*['\"]termios['\"]"),
    re.compile(r"\b__import__\s*\(\s*['\"]signal['\"]"),
    re.compile(r"\b__import__\s*\(\s*['\"]multiprocessing['\"]"),
    re.compile(r"\b__import__\s*\(\s*['\"]codeop['\"]"),
    re.compile(r"\b__import__\s*\(\s*['\"]compileall['\"]"),
    re.compile(r"\bexec\s*\(\s*['\"]"),
    re.compile(r"\beval\s*\(\s*['\"]"),
    re.compile(r"\bcompile\s*\(\s*['\"]"),
    re.compile(r"\bopen\s*\(\s*['\"](/etc|/proc|/sys|/dev)"),
    # 绕过变体检测
    re.compile(r"\bgetattr\s*\(\s*__builtins__\s*,\s*['\"]exec['\"]"),
    re.compile(r"\bgetattr\s*\(\s*__builtins__\s*,\s*['\"]eval['\"]"),
    re.compile(r"\bgetattr\s*\(\s*__builtins__\s*,\s*['\"]compile['\"]"),
    re.compile(r"\bgetattr\s*\(\s*__builtins__\s*,\s*['\"]open['\"]"),
    re.compile(r"\bvars\s*\(\s*\)\s*\[\s*['\"]exec['\"]"),
    re.compile(r"\bvars\s*\(\s*\)\s*\[\s*['\"]eval['\"]"),
    re.compile(r"\bexec\s*\(\s*compile\s*\("),
    re.compile(r"\beval\s*\(\s*compile\s*\("),
    re.compile(r"\bexec\s*\(\s*base64"),
    re.compile(r"\beval\s*\(\s*base64"),
    re.compile(r"\bexec\s*\(\s*codecs"),
    re.compile(r"\beval\s*\(\s*codecs"),
    re.compile(r"\bexec\s*\(\s*bytes\s*\("),
    re.compile(r"\beval\s*\(\s*bytes\s*\("),
    re.compile(r"\bexec\s*\(\s*bytearray\s*\("),
    re.compile(r"\beval\s*\(\s*bytearray\s*\("),
    re.compile(r"\bexec\s*\(\s*memoryview\s*\("),
    re.compile(r"\beval\s*\(\s*memoryview\s*\("),
    re.compile(r"\bexec\s*\(\s*hex\s*\("),
    re.compile(r"\beval\s*\(\s*hex\s*\("),
    re.compile(r"\bexec\s*\(\s*oct\s*\("),
    re.compile(r"\beval\s*\(\s*oct\s*\("),
    re.compile(r"\bexec\s*\(\s*bin\s*\("),
    re.compile(r"\beval\s*\(\s*bin\s*\("),
    re.compile(r"\bexec\s*\(\s*chr\s*\("),
    re.compile(r"\beval\s*\(\s*chr\s*\("),
    re.compile(r"\bexec\s*\(\s*ord\s*\("),
    re.compile(r"\beval\s*\(\s*ord\s*\("),
    re.compile(r"\bexec\s*\(\s*str\s*\("),
    re.compile(r"\beval\s*\(\s*str\s*\("),
    re.compile(r"\bexec\s*\(\s*repr\s*\("),
    re.compile(r"\beval\s*\(\s*repr\s*\("),
    re.compile(r"\bexec\s*\(\s*ascii\s*\("),
    re.compile(r"\beval\s*\(\s*ascii\s*\("),
    re.compile(r"\bexec\s*\(\s*format\s*\("),
    re.compile(r"\beval\s*\(\s*format\s*\("),
    re.compile(r"\bexec\s*\(\s*join\s*\("),
    re.compile(r"\beval\s*\(\s*join\s*\("),
    re.compile(r"\bexec\s*\(\s*split\s*\("),
    re.compile(r"\beval\s*\(\s*split\s*\("),
    re.compile(r"\bexec\s*\(\s*replace\s*\("),
    re.compile(r"\beval\s*\(\s*replace\s*\("),
    re.compile(r"\bexec\s*\(\s*strip\s*\("),
    re.compile(r"\beval\s*\(\s*strip\s*\("),
    re.compile(r"\bexec\s*\(\s*lstrip\s*\("),
    re.compile(r"\beval\s*\(\s*lstrip\s*\("),
    re.compile(r"\bexec\s*\(\s*rstrip\s*\("),
    re.compile(r"\beval\s*\(\s*rstrip\s*\("),
    re.compile(r"\bexec\s*\(\s*upper\s*\("),
    re.compile(r"\beval\s*\(\s*upper\s*\("),
    re.compile(r"\bexec\s*\(\s*lower\s*\("),
    re.compile(r"\beval\s*\(\s*lower\s*\("),
    re.compile(r"\bexec\s*\(\s*title\s*\("),
    re.compile(r"\beval\s*\(\s*title\s*\("),
    re.compile(r"\bexec\s*\(\s*capitalize\s*\("),
    re.compile(r"\beval\s*\(\s*capitalize\s*\("),
    re.compile(r"\bexec\s*\(\s*swapcase\s*\("),
    re.compile(r"\beval\s*\(\s*swapcase\s*\("),
    re.compile(r"\bexec\s*\(\s*encode\s*\("),
    re.compile(r"\beval\s*\(\s*encode\s*\("),
    re.compile(r"\bexec\s*\(\s*decode\s*\("),
    re.compile(r"\beval\s*\(\s*decode\s*\("),
    re.compile(r"\bexec\s*\(\s*translate\s*\("),
    re.compile(r"\beval\s*\(\s*translate\s*\("),
    re.compile(r"\bexec\s*\(\s*maketrans\s*\("),
    re.compile(r"\beval\s*\(\s*maketrans\s*\("),
    re.compile(r"\bexec\s*\(\s*zfill\s*\("),
    re.compile(r"\beval\s*\(\s*zfill\s*\("),
    re.compile(r"\bexec\s*\(\s*center\s*\("),
    re.compile(r"\beval\s*\(\s*center\s*\("),
    re.compile(r"\bexec\s*\(\s*ljust\s*\("),
    re.compile(r"\beval\s*\(\s*ljust\s*\("),
    re.compile(r"\bexec\s*\(\s*rjust\s*\("),
    re.compile(r"\beval\s*\(\s*rjust\s*\("),
    re.compile(r"\bexec\s*\(\s*expandtabs\s*\("),
    re.compile(r"\beval\s*\(\s*expandtabs\s*\("),
    re.compile(r"\bexec\s*\(\s*partition\s*\("),
    re.compile(r"\beval\s*\(\s*partition\s*\("),
    re.compile(r"\bexec\s*\(\s*rpartition\s*\("),
    re.compile(r"\beval\s*\(\s*rpartition\s*\("),
    re.compile(r"\bexec\s*\(\s*splitlines\s*\("),
    re.compile(r"\beval\s*\(\s*splitlines\s*\("),
    re.compile(r"\bexec\s*\(\s*rsplit\s*\("),
    re.compile(r"\beval\s*\(\s*rsplit\s*\("),
    re.compile(r"\bexec\s*\(\s*startswith\s*\("),
    re.compile(r"\beval\s*\(\s*startswith\s*\("),
    re.compile(r"\bexec\s*\(\s*endswith\s*\("),
    re.compile(r"\beval\s*\(\s*endswith\s*\("),
    re.compile(r"\bexec\s*\(\s*find\s*\("),
    re.compile(r"\beval\s*\(\s*find\s*\("),
    re.compile(r"\bexec\s*\(\s*rfind\s*\("),
    re.compile(r"\beval\s*\(\s*rfind\s*\("),
    re.compile(r"\bexec\s*\(\s*index\s*\("),
    re.compile(r"\beval\s*\(\s*index\s*\("),
    re.compile(r"\bexec\s*\(\s*rindex\s*\("),
    re.compile(r"\beval\s*\(\s*rindex\s*\("),
    re.compile(r"\bexec\s*\(\s*count\s*\("),
    re.compile(r"\beval\s*\(\s*count\s*\("),
    re.compile(r"\bexec\s*\(\s*isalnum\s*\("),
    re.compile(r"\beval\s*\(\s*isalnum\s*\("),
    re.compile(r"\bexec\s*\(\s*isalpha\s*\("),
    re.compile(r"\beval\s*\(\s*isalpha\s*\("),
    re.compile(r"\bexec\s*\(\s*isdigit\s*\("),
    re.compile(r"\beval\s*\(\s*isdigit\s*\("),
    re.compile(r"\bexec\s*\(\s*isspace\s*\("),
    re.compile(r"\beval\s*\(\s*isspace\s*\("),
    re.compile(r"\bexec\s*\(\s*istitle\s*\("),
    re.compile(r"\beval\s*\(\s*istitle\s*\("),
    re.compile(r"\bexec\s*\(\s*isupper\s*\("),
    re.compile(r"\beval\s*\(\s*isupper\s*\("),
    re.compile(r"\bexec\s*\(\s*islower\s*\("),
    re.compile(r"\beval\s*\(\s*islower\s*\("),
    re.compile(r"\bexec\s*\(\s*isidentifier\s*\("),
    re.compile(r"\beval\s*\(\s*isidentifier\s*\("),
    re.compile(r"\bexec\s*\(\s*isprintable\s*\("),
    re.compile(r"\beval\s*\(\s*isprintable\s*\("),
    re.compile(r"\bexec\s*\(\s*isnumeric\s*\("),
    re.compile(r"\beval\s*\(\s*isnumeric\s*\("),
    re.compile(r"\bexec\s*\(\s*isdecimal\s*\("),
    re.compile(r"\beval\s*\(\s*isdecimal\s*\("),
    re.compile(r"\bexec\s*\(\s*isascii\s*\("),
    re.compile(r"\beval\s*\(\s*isascii\s*\("),
    # Bash 危险命令
...
class CodeScanner:
    ...
    def scan_bash(cls, command: str, **kw) -> Tuple[bool, str]:
        ...
        # 白名单检查 (如果命令不在白名单中, 拦截执行)
        if cmd_name and cmd_name not in SAFE_BASH_COMMANDS:
            logger.warning(f"未知命令 '{cmd_name}' (不在白名单中), 已拦截")
