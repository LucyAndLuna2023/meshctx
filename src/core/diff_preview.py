...
=======
    def compute_diff(self, old_text: str, new_text: str, filename: str = "") -> str:
        """🔧 BUG-043-fix: Alias for :meth:`generate` — canonical diff name.

        Called by /api/diff endpoint on the hot path.
        """
        return self.generate(old_text, new_text, filename)

    def render_side_by_side(self, old_text: str, new_text: str, width: int = 80) -> str:
        """🔧 BUG-044-fix: Render old ↔ new in side-by-side columns.

        Each line is printed as ``old │ new`` with trimmed text.  The
        shorter side is padded to the matching line count.
        """
        ...
        for i in range(max_n):
            ...
            left = left[: width // 2 - 2]
            right = right[: width // 2 - 2]
            lines.append(f"{left:<{width//2-1}}│ {right}")
        return "\n".join(lines)
    ...
    def render_compact_summary(
        ...
        """🔧 BUG-045-fix: Compact human-readable summary of one or more diffs.

        Args:
            diffs: List of unified-diff strings.
            max_lines: Maximum total lines in the summary output.
            context_lines: Context lines kept around each hunk marker.

        Returns:
            A shortened diff string suitable for chat previews.
        """
        if not diffs:
            return "(no changes)"
        compact: list[str] = []
        for diff in diffs:
            ...
            for line in diff.splitlines():
                if line.startswith("@@") or line.startswith("---") or line.startswith("+++"):
                    if context_buf:
                        compact.extend(context_buf[-context_lines:])
                        context_buf.clear()
                    compact.append(line)
                    hunk = True
        ...
        if len(compact) > max_lines:
            ...
            compact.append(f"... ({len(diffs)} file(s), truncated)")
        return "\n".join(compact) if compact else "(no changes)"
