...
class ProxyManager:
    ...
    def add_backend(self, host: str, port: int, weight: float = 1.0) -> dict:
        """添加后端服务器."""
    ...
    def mark_unhealthy(self, host: str, port: int) -> bool:
        """标记后端为不健康."""
    ...
    def mark_healthy(self, host: str, port: int) -> bool:
        """标记后端为健康."""
        for b in self._backends:
            if b["host"] == host and b["port"] == port:
                b["healthy"] = True
    ...
    def add_route(self, path: str, backend: str) -> None:
        """添加路由."""
    ...
    def round_robin(self) -> dict | None:
        """轮询选择后端."""
    ...
    def weighted_select(self) -> dict | None:
        """加权选择后端."""
        healthy = self.get_healthy_backends()
        if not healthy:
            return None
    ...
    def proxy_request(self, path: str, method: str = "GET") -> dict:
        """代理 HTTP 请求."""
        self._request_count += 1
