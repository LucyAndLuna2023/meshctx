...
async def api_chat_stream(request: Request):
    ...
    async def generate():
        try:
            ...
            _tools_ok = True
            _start_ts = time.time()
            _total_search_calls = 0  # 防循环: 总搜索次数上限（替代被删的 b0d65ea 防循环）
            for _round in range(max_rounds):
                ...
                if msg.tool_calls:
                    ...
                    def _exec_one(name, args):
                        ...
                        if name == "web_search":
                            ...
                            if _total_search_calls > 8:
                                return ("[防循环] 搜索已达 8 次上限，请立即基于已有信息输出最终结果，"
                                        "不要再调用 web_search")
                            return _do_web_search(args.get("query", ""))
                        elif name == "web_extract":
                            return _do_web_extract(args.get("url", ""))
...
def _do_terminal(cmd: str, workdir: str = "", timeout: int = 60) -> str:
    """在本机执行 shell 命令 → 委托 chat_tools（含危险命令检测）"""
    # 危险命令检测 — 与 /api/terminal 一致，用 sandbox 统一验证
