                ...
                if msg.tool_calls:
                    ...
                    for tc in msg.tool_calls:
                        ...
                        if name == "web_search":
                            ...
                            result = _do_terminal(args.get("cmd", ""), args.get("workdir", ""),
                                                  args.get("timeout", 60))
                        elif name == "remote_read":
...
async def ai_monitor_provider_stats():
    """AI监控 — 供应商统计"""
    try:
        from .core.usage_insights import get_insights
        insights = get_insights()
        return {
            "providers": len(insights.get("providers", {})),
            "models": sum(len(v) for v in insights.get("providers", {}).values()),
            "status": "monitoring",
        }
    except Exception:
...
async def recovery_plan_status():
    """恢复计划 — 状态"""
    try:
        from .core.error_recovery import get_recovery_plans
