...
{% elif flash == "error" %}
<div class="flash flash-error">❌ {{ t('mdl_op_failed') }}</div>
{% elif flash == "deleted" %}
<div class="flash flash-success">🗑 {{ t("deleted_ok") }}</div>
{% endif %}

<div style="display:flex;justify-content:space-between;align-items:center;margin:16px 0;">
    <h3 style="margin:0;">{{ t('mdl_configured_models') }} <span style="color:var(--accent);" id="modelCount">{{ configured|length }}</span></h3>
    <button class="btn btn-primary" onclick="showAddForm()" style="padding:10px 20px;">+ {{ t('mdl_add_model') }}</button>
</div>

<!-- v2.17: 本地模型快捷预设 -->
<div style="margin-bottom:12px;display:flex;flex-wrap:wrap;gap:6px;">
    <span style="font-size:12px;color:var(--muted);line-height:28px;">{{ t('mdl_quick_presets') }}</span>
...
<div id="modelForm" style="display:none;margin-bottom:16px;">
    <div class="card">
        <h3 id="formTitle">{{ t('mdl_add_model') }}</h3>
        ...
        <div class="form-group"><label>{{ t('mdl_model_name_opt') }}</label><input id="fmodel" placeholder="auto"></div>
        <div class="form-group"><label>{{ t('mdl_base_url_opt') }}</label><input id="furl" placeholder="auto"></div>
        <div style="display:flex;gap:8px;">
            <button class="btn btn-primary" onclick="saveModel()">💾 {{ t("save") }}</button>
            <button class="btn btn-ghost" onclick="hideForm()">{{ t('mdl_cancel') }}</button>
            <button class="btn btn-ghost" onclick="testFromForm()" style="margin-left:auto;">🔍 {{ t('mdl_test_conn') }}</button>
        </div>
...
<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px;">
    <div>
        <span style="color:var(--muted);font-size:12px;">
            🟢 <b id="readyCount">{{ all_models|selectattr('ready')|list|length }}</b> {{ t('mdl_ready') }}
            &nbsp;🔴 <b id="unreadyCount">{{ all_models|rejectattr('ready')|list|length }}</b> {{ t('mdl_unready') }}
        </span>
    </div>
    <button class="btn btn-ghost" style="font-size:11px;padding:4px 10px;color:#f85149;" onclick="cleanUnconfigured()">🗑 {{ t('mdl_clean_unconfigured') }}</button>
</div>
{% if has_more_unconfigured %}
<div style="text-align:center;margin-bottom:12px;">
    <span style="color:var(--muted);font-size:11px;">{{ t('mdl_show_limited') }} ({{ total_unconfigured }})</span>
    <a href="?all=1" style="color:var(--accent);font-size:11px;margin-left:8px;">{{ t('mdl_expand_all') }}</a>
</div>
{% endif %}
<table style="width:100%;border-collapse:collapse;font-size:13px;">
<thead><tr style="border-bottom:1px solid var(--border);text-align:left;color:var(--muted);">
    <th style="padding:8px;">{{ t('mdl_col_status') }}</th>
    <th style="padding:8px;">{{ t('mdl_col_model_id') }}</th>
    <th style="padding:8px;">{{ t('mdl_col_provider') }}</th>
    <th style="padding:8px;">{{ t('mdl_col_endpoint') }}</th>
    <th style="padding:8px;">Key</th>
    <th style="padding:8px;">{{ t('mdl_col_actions') }}</th>
</tr></thead>
<tbody>
{% for m in all_models %}
...
<tr style="border-bottom:1px solid var(--border);{% if is_def %}background:rgba(108,92,231,0.08);{% endif %}" data-id="{{ m.id }}">
    <td style="padding:8px;">
        {% if is_def %}
        <span style="background:var(--accent);color:#fff;padding:2px 8px;border-radius:10px;font-size:10px;font-weight:700;">⭐ {{ t('mdl_default_short') }}</span>
        {% elif is_ready %}
        <span style="color:#22c55e;font-size:11px;">🟢 {{ t('mdl_ready') }}</span>
        {% else %}
        <span style="color:#f85149;font-size:11px;">🔴 {{ t('mdl_unready') }}</span>
        {% endif %}
    </td>
    <td style="padding:8px;">
        <strong>{{ m.id }}</strong>
    ...
    <td style="padding:8px;">
        ...
        {% elif is_ready %}
        <span style="color:var(--muted);font-size:10px;">{{ t('mdl_default_short') }}</span>
        {% else %}
        <span style="color:#f85149;font-size:10px;">{{ t('mdl_not_set') }}</span>
        {% endif %}
    </td>
    <td style="padding:8px;">
        {% if m.key_masked %}
    ...
    <td style="padding:8px;">
        <div style="display:flex;gap:4px;">
            ...
            {% if not is_def %}
            <button class="btn btn-ghost" style="font-size:11px;padding:2px 8px;color:#22c55e;" onclick="setDefault('{{ m.id }}')">⭐{{ t('mdl_default_short') }}</button>
            {% endif %}
            {% else %}
            <button class="btn btn-ghost" style="font-size:11px;padding:2px 8px;color:var(--accent);" onclick="configureModel('{{ m.id }}')">⚡ {{ t('mdl_configure') }}</button>
            {% endif %}
            {% if not is_def %}
            <button class="btn btn-ghost" style="font-size:11px;padding:2px 8px;color:#f85149;" onclick="if(confirm(window.__t('mdl_confirm_delete')+' {{ m.id }}?'))deleteModel('{{ m.id }}')">✕</button>
            {% endif %}
        </div>
    </td>
</tr>
...
<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:12px;margin-top:16px;">
    <a href="https://platform.deepseek.com/api_keys" target="_blank" class="card" style="text-align:center;text-decoration:none;color:inherit;padding:12px;">
        <span>🟢 DeepSeek</span><br><span style="color:#38bdf8;font-size:11px;">{{ t('mdl_get_key') }}</span>
    </a>
    <a href="https://bailian.console.aliyun.com/" target="_blank" class="card" style="text-align:center;text-decoration:none;color:inherit;padding:12px;">
        <span>🔵 {{ t('mdl_brand_bailian') }}</span><br><span style="color:#38bdf8;font-size:11px;">{{ t('mdl_get_key') }}</span>
    </a>
    <a href="https://siliconflow.cn/" target="_blank" class="card" style="text-align:center;text-decoration:none;color:inherit;padding:12px;">
        <span>🔴 {{ t('mdl_brand_siliconflow') }}</span><br><span style="color:#38bdf8;font-size:11px;">{{ t('mdl_get_key') }}</span>
    </a>
</div>

<script>
if (!window.__t) { window.__t = function(k){ var lg=(document.documentElement.lang||'en').split('-')[0]; var L=(window.LANG||{})[lg]||(window.LANG||{}).en||{}; return L[k]||k; }; }
</script>
<script>
function showAddForm() {
    document.getElementById('modelForm').style.display = 'block';
    document.getElementById('formTitle').textContent = window.__t('mdl_add_model');
...
function editModel(id, provider, key, model, url) {
    showAddForm();
    document.getElementById('formTitle').textContent = window.__t('mdl_edit') + ' ' + id;
...
async function saveModel() {
    ...
    if (!body.id || !body.provider) { alert(window.__t('mdl_err_required')); return; }
    if (!body.key && !body.base_url) { alert(window.__t('mdl_err_key_or_url')); return; }
    ...
    try {
        ...
        else { alert(window.__t('mdl_err_failed') + (data.detail||data.message||JSON.stringify(data))); }
    } catch(e) { alert(window.__t('mdl_err_network') + e.message); }
}
async function deleteModel(id) {
    try {
        var res = await fetch('/api/models/' + id, {method: 'DELETE'});
        if (res.ok) location.reload();
        else { var d = await res.json(); alert(window.__t('mdl_err_failed') + (d.detail||'')); }
    } catch(e) { alert(window.__t('mdl_err') + e.message); }
}
...
async function cleanUnconfigured() {
    if (!confirm(window.__t('mdl_confirm_clean'))) return;
    try {
        var res = await fetch('/api/models/clean-unconfigured', {method: 'POST'});
        var d = await res.json();
    alert(window.__t('mdl_cleaned_prefix')+' '+(d.deleted || 0)+' '+window.__t('mdl_cleaned_suffix'));
        location.reload();
    } catch(e) { alert(window.__t('mdl_err') + e.message); }
}
function configureModel(id) {
    ...
    document.getElementById('fid').disabled = false;
    document.getElementById('editModelId').value = id;
    document.getElementById('formTitle').textContent = window.__t('mdl_configure_key') + ' — ' + id;
...
async function testModel(id) {
    ...
    try {
        ...
        var d = await res.json();
        if (d.status === 'ok') alert('✅ ' + id + ' ' + window.__t('mdl_test_ok'));
        else alert('❌ ' + id + ': ' + (d.message||window.__t('mdl_err_failed')));
    } catch(e) { alert(window.__t('mdl_err') + e.message); }
    if (tr) tr.style.background = '';
}
async function testFromForm() {
    var id = document.getElementById('fid').value.trim();
    if (!id) { alert(window.__t('mdl_need_id')); return; }
    document.getElementById('testResult').innerHTML = '⏳ ' + window.__t('mdl_testing');
    try {
        var res = await fetch('/api/models/' + id + '/test', {method: 'POST'});
        var d = await res.json();
        document.getElementById('testResult').innerHTML = d.status === 'ok' 
            ? '<span style="color:#22c55e;">✅ ' + d.message + '</span>'
            : '<span style="color:#f85149;">❌ ' + (d.message||window.__t('mdl_err_failed')) + '</span>';
    } catch(e) { document.getElementById('testResult').innerHTML = '<span style="color:#f85149;">' + window.__t('mdl_err') + ': ' + e.message + '</span>'; }
}
</script>
{% endblock %}"""
