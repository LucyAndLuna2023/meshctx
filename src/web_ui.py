...
async def dashboard(request: Request):
    """v3.115.31: Redirect to v2 dashboard (2-entry + gear UI)"""
    return RedirectResponse(url="/ui/v2", status_code=302)
...
# ═══════════════════════════════════════════════════════════════
# v2.15 UI REFACTOR — 11 tab → 2 入口 + 齿轮
# 新增模板和路由，旧版保留兼容
# ═══════════════════════════════════════════════════════════════
...
async def dashboard_v2(request: Request):
    return _render("dashboard_v2.html", {"request": request, "title": "meshctx"}, request)

# ── v2 Chat 页 (带历史侧栏) ──
@router.get("/v2/chat", response_class=HTMLResponse)
async def chat_v2(request: Request):
    return _render("chat_v2.html", {"request": request, "title": "Chat — meshctx"}, request)

# ── v2 Projects 页 ──
@router.get("/v2/projects", response_class=HTMLResponse)
async def projects_v2(request: Request):
    return _render("projects_v2.html", {"request": request, "title": "Projects — meshctx"}, request)

# ── v2 设置面板 ──
@router.get("/v2/settings", response_class=HTMLResponse)
async def settings_v2(request: Request):
    return _render("settings_v2.html", {"request": request, "title": "设置 — meshctx"}, request)

# ── v2 开发者面板 ──
@router.get("/v2/dev", response_class=HTMLResponse)
async def dev_v2(request: Request):
    return _render("dev_v2.html", {"request": request, "title": "开发者工具 — meshctx"}, request)

# ═══════════════════════════════════════════════════════════════
# v2 TEMPLATES — 2 Entry + Gear UI (Codex Audit Optimization)
# ═══════════════════════════════════════════════════════════════

_TEMPLATES["dashboard_v2.html"] = """<!DOCTYPE html>
<html lang="zh-CN">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>{{ title }}</title>
    <style>
        :root {--bg: #0f172a; --text: #e2e8f0; --muted: #94a3b8; --surface: #1e293b; --border: #334155; --accent: #6c5ce7; --accent-glow: rgba(108,92,231,0.3); --radius: 12px; --shadow: 0 4px 24px rgba(0,0,0,0.3);}
        [data-theme="light"] {--bg: #f8fafc; --text: #0f172a; --muted: #64748b; --surface: #ffffff; --border: #e2e8f0; --shadow: 0 4px 24px rgba(0,0,0,0.08);}
        * {margin:0; padding:0; box-sizing:border-box;}
        body {font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', system-ui, sans-serif; background: var(--bg); color: var(--text); min-height: 100vh; display: flex; flex-direction: column; align-items: center; justify-content: center; transition: background 0.3s, color 0.3s;}
        .container {max-width: 600px; width: 100%; padding: 32px 24px;}
        .header {text-align: center; margin-bottom: 40px;}
        .logo-text {font-size: 32px; font-weight: 700; background: linear-gradient(135deg, #6c5ce7, #06b6d4); -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text;}
        ...
        .card {background: var(--surface); border: 1px solid var(--border); border-radius: var(--radius); padding: 28px 20px; text-align: center; cursor: pointer; transition: all 0.25s; text-decoration: none; color: var(--text); box-shadow: var(--shadow); display: block;}
        .card:hover {border-color: var(--accent); box-shadow: 0 0 24px var(--accent-glow); transform: translateY(-2px);}
        ...
        .gear-btn {position: absolute; top: 20px; right: 20px; width: 44px; height: 44px; border-radius: 50%; border: 1px solid var(--border); background: var(--surface); color: var(--text); font-size: 20px; cursor: pointer; display: flex; align-items: center; justify-content: center; box-shadow: var(--shadow);}
        .gear-btn:hover {border-color: var(--accent); box-shadow: 0 0 16px var(--accent-glow); transform: rotate(30deg);}
        .theme-btn {position: absolute; top: 20px; left: 20px; width: 44px; height: 44px; border-radius: 50%; border: 1px solid var(--border); background: var(--surface); color: var(--text); font-size: 18px; cursor: pointer; display: flex; align-items: center; justify-content: center; box-shadow: var(--shadow);}
        ...
        .modal {background: var(--surface); border: 1px solid var(--border); border-radius: var(--radius); padding: 28px; max-width: 420px; width: 90%; box-shadow: 0 20px 60px rgba(0,0,0,0.5);}
        .modal h3 {font-size: 18px; margin-bottom: 20px;}
        .modal-item {display: flex; align-items: center; gap: 12px; padding: 14px 16px; border-radius: 8px; cursor: pointer; text-decoration: none; color: var(--text); margin-bottom: 4px;}
        .modal-item:hover {background: #1a2332;}
        .modal-item .mi-icon {font-size: 20px; width: 28px; text-align: center;}
        .modal-item .mi-label {font-weight: 500; font-size: 14px;}
        .modal-item .mi-desc {font-size: 12px; color: var(--muted);}
        .modal-close {float: right; background: none; border: none; color: var(--muted); font-size: 22px; cursor: pointer; padding: 4px 8px;}
        .modal-close:hover {color: var(--text);}
        .dev-hint {margin-top: 20px; font-size: 11px; color: var(--muted); text-align: center;}
        .dev-hint kbd {background: #1a2332; border: 1px solid var(--border); border-radius: 4px; padding: 2px 6px; font-family: monospace; font-size: 11px;}
    </style>
</head>
<body data-theme="dark">
    ...
    <div class="container">
        <div class="header"><div class="logo-text">&#129504; meshctx</div><div class="subtitle">AI Native Dev Workbench v2.0</div></div>
        <div class="cards">
            <a href="/ui/v2/chat" class="card"><div class="card-icon">&#128172;</div><div class="card-title">Chat</div><div class="card-desc">AI Assistant &middot; Multi-model &middot; Context Memory</div></a>
            <a href="/ui/v2/projects" class="card"><div class="card-icon">&#128194;</div><div class="card-title">Projects</div><div class="card-desc">Project Mgmt &middot; Sessions &middot; Memory &middot; Continuity</div></a>
        </div>
    </div>
    <div class="modal-overlay" id="settingsModal" onclick="closeSettings(event)">
        <div class="modal" onclick="event.stopPropagation()">
            ...
            <h3>&#9881; Settings</h3>
            <a href="/ui/providers" class="modal-item"><span class="mi-icon">&#128268;</span><div><div class="mi-label">Model Management</div><div class="mi-desc">API Providers &middot; Keys &middot; Connectivity</div></div></a>
            <a href="/ui/plugins" class="modal-item"><span class="mi-icon">&#129521;</span><div><div class="mi-label">Plugin Marketplace</div><div class="mi-desc">Install &middot; Enable &middot; Community</div></div></a>
            <a href="/ui/dashboard" class="modal-item"><span class="mi-icon">&#128202;</span><div><div class="mi-label">System Status</div><div class="mi-desc">Monitor &middot; Resources &middot; Health</div></div></a>
            <a href="/ui/memory" class="modal-item"><span class="mi-icon">&#128190;</span><div><div class="mi-label">Memory Management</div><div class="mi-desc">Search &middot; Graph &middot; Cleanup</div></div></a>
            <div class="dev-hint">Dev Tools: Press <kbd>Ctrl+Shift+D</kbd> or visit <a href="/ui/v2/dev" style="color:var(--accent);">Dev Panel</a></div>
        </div>
    </div>
    <div class="footer">meshctx v2.15 &middot; <a href="/ui/v2/dev" style="color:var(--muted);">Dev</a></div>
    <script>
        function toggleTheme(){var b=document.body;var t=b.getAttribute('data-theme')==='light'?'dark':'light';b.setAttribute('data-theme',t);localStorage.setItem('meshctx_theme',t);}
        function openSettings(){document.getElementById('settingsModal').classList.add('active');}
        function closeSettings(e){if(e&&e.target!==document.getElementById('settingsModal'))return;document.getElementById('settingsModal').classList.remove('active');}
        document.addEventListener('keydown',function(e){if(e.ctrlKey&&e.shiftKey&&e.key==='D'){e.preventDefault();window.location.href='/ui/v2/dev';}});
        (function(){var t=localStorage.getItem('meshctx_theme');if(t)document.body.setAttribute('data-theme',t);})();
    </script>
</body>
</html>
"""

_TEMPLATES["chat_v2.html"] = """<!DOCTYPE html>
<html lang="zh-CN">
<head>
...
<body>
    <div class="topbar">
        ...
        <button class="toggle-sidebar" onclick="toggleSidebar()" title="History">&#128240; History</button>
    </div>
    <div class="layout">
        <div class="sidebar" id="chatSidebar">
            <div class="sidebar-header">&#128240; History <span class="count" id="historyCount"></span></div>
            <button class="new-chat-btn" onclick="newChat()">+ New Chat</button>
            <div class="sidebar-list" id="historyList"><div style="padding:16px;color:var(--muted);">Loading...</div></div>
        </div>
        <iframe src="/ui/chat" class="chat-embed" id="chatFrame"></iframe>
    </div>
    <script>
        function toggleSidebar(){var s=document.getElementById('chatSidebar');if(window.innerWidth<=768){s.classList.toggle('mobile-open');}else{s.classList.toggle('collapsed');}}
        function newChat(){document.getElementById('chatFrame').src='/ui/chat';setTimeout(loadHistory,500);}
        function loadHistory(){var list=document.getElementById('historyList'),count=document.getElementById('historyCount');try{var tabId=localStorage.getItem('meshctx_active_tab')||'default';var data=JSON.parse(localStorage.getItem('meshctx_chat_'+tabId)||'[]');count.textContent=data.length;list.innerHTML=data.slice(-30).reverse().map(function(m){var p=(m.content||'').substring(0,40).replace(/</g,'&lt;');return '<div class="sidebar-item">'+p+'</div>';}).join('')||'<div style="padding:16px;color:var(--muted);">No history</div>';}catch(e){list.innerHTML='<div style="padding:16px;color:var(--muted);">Load failed</div>';}}
        loadHistory();setInterval(loadHistory,8000);
    </script>
</body>
</html>
"""

_TEMPLATES["settings_v2.html"] = """<!DOCTYPE html>
<html lang="zh-CN">
<head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>{{ title }}</title>
<style>
    :root {--bg:#0f172a; --text:#e2e8f0; --surface:#1e293b; --border:#334155; --accent:#6c5ce7; --muted:#94a3b8;}
    * {margin:0;padding:0;box-sizing:border-box;}
    ...
    .card {background:var(--surface);border:1px solid var(--border);border-radius:10px;padding:20px;cursor:pointer;transition:all .2s;text-decoration:none;color:var(--text);display:block;}
...
<body>
<div class="header"><a href="/ui/v2">&larr;</a><h2>&#9881; Settings</h2></div>
<div class="grid">
    <a href="/ui/providers" class="card"><div class="icon">&#128268;</div><div class="name">Model Mgmt</div><div class="desc">API Providers &middot; Keys &middot; Connectivity</div></a>
    <a href="/ui/plugins" class="card"><div class="icon">&#129521;</div><div class="name">Plugins</div><div class="desc">Install &middot; Enable &middot; Marketplace</div></a>
    <a href="/ui/dashboard" class="card"><div class="icon">&#128202;</div><div class="name">System Status</div><div class="desc">Monitor &middot; Resources &middot; Health</div></a>
    <a href="/ui/memory" class="card"><div class="icon">&#128190;</div><div class="name">Memory</div><div class="desc">Search &middot; Graph &middot; Stats</div></a>
    <a href="/ui/continuity" class="card"><div class="icon">&#128279;</div><div class="name">Continuity</div><div class="desc">Project Continuity Score</div></a>
    <a href="/ui/files" class="card"><div class="icon">&#128193;</div><div class="name">File Manager</div><div class="desc">Project file browser</div></a>
</div>
<div style="margin-top:24px;text-align:center;font-size:12px;color:var(--muted);">Dev Tools: <a href="/ui/v2/dev" style="color:var(--accent);">Dev Panel</a> or Ctrl+Shift+D</div>
</body>
</html>
"""

_TEMPLATES["dev_v2.html"] = """<!DOCTYPE html>
<html lang="zh-CN">
<head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>{{ title }}</title>
<style>
    :root {--bg:#0f172a;--text:#e2e8f0;--surface:#1e293b;--border:#334155;--accent:#6c5ce7;--muted:#94a3b8;--orange:#f97316;}
    * {margin:0;padding:0;box-sizing:border-box;}
    body {font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',system-ui,sans-serif;background:var(--bg);color:var(--text);padding:20px;}
    .header {display:flex;align-items:center;gap:12px;margin-bottom:24px;border-bottom:1px solid var(--orange);padding-bottom:12px;}
    .header a {color:var(--muted);text-decoration:none;font-size:18px;}
    .header h2 {font-size:18px;color:var(--orange);}
    .warning {background:rgba(249,115,22,0.1);border:1px solid var(--orange);border-radius:8px;padding:12px;font-size:12px;margin-bottom:20px;}
    .grid {display:grid;grid-template-columns:repeat(auto-fill,minmax(280px,1fr));gap:12px;}
    .card {background:var(--surface);border:1px solid var(--border);border-radius:10px;padding:20px;cursor:pointer;transition:all .2s;text-decoration:none;color:var(--text);display:block;}
...
<body>
<div class="header"><a href="/ui/v2">&larr;</a><h2>&#128736; Dev Tools</h2></div>
<div class="warning">&#9888; Developer Panel &mdash; experimental/low-level features. Changes may affect system stability.</div>
<div class="grid">
    <a href="/ui/?tab=brain" class="card"><div class="icon">&#129504;</div><div class="name">Brain Map</div><div class="desc">Brain regions &middot; Neuron connections</div></a>
    <a href="/ui/?tab=agent" class="card"><div class="icon">&#129302;</div><div class="name">Agent Mgmt</div><div class="desc">OODA loop &middot; Task monitor</div></a>
    <a href="/ui/?tab=lab" class="card"><div class="icon">&#129514;</div><div class="name">Lab</div><div class="desc">Feature testing &middot; Canary</div></a>
    <a href="/ui/?tab=sandbox-dt" class="card"><div class="icon">&#128421;</div><div class="name">Sandbox</div><div class="desc">Execution env &middot; Isolation</div></a>
    <a href="/ui/?tab=win-dt" class="card"><div class="icon">&#129695;</div><div class="name">Processes</div><div class="desc">Running processes &middot; Resources</div></a>
    <a href="/ui/?tab=monitor" class="card"><div class="icon">&#128202;</div><div class="name">Full Monitor</div><div class="desc">Complete system dashboard</div></a>
</div>
</body>
</html>
"""

_TEMPLATES["projects_v2.html"] = """<!DOCTYPE html>
<html lang="zh-CN">
<head>
    <meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>{{ title }}</title>
    <style>
        :root {--bg:#0f172a;--text:#e2e8f0;--muted:#94a3b8;--surface:#1e293b;--border:#334155;--accent:#6c5ce7;--hover:#1a2332;--radius:10px;}
        * {margin:0;padding:0;box-sizing:border-box;}
        ...
        .header .new-btn {padding:8px 16px;background:var(--accent);color:#fff;border:none;border-radius:6px;cursor:pointer;font-size:13px;font-weight:600;}
        .grid {display:grid;grid-template-columns:repeat(auto-fill,minmax(300px,1fr));gap:16px;}
        .card {background:var(--surface);border:1px solid var(--border);border-radius:var(--radius);padding:20px;cursor:pointer;transition:all .2s;text-decoration:none;color:var(--text);display:block;}
        .card:hover {border-color:var(--accent);}
        .card .name {font-weight:600;font-size:16px;margin-bottom:6px;}
        .card .desc {font-size:13px;color:var(--muted);margin-bottom:12px;}
        .card .meta {display:flex;gap:16px;font-size:11px;color:var(--muted);}
        .card .status {display:inline-block;padding:2px 8px;border-radius:10px;font-size:10px;font-weight:500;}
...
<div class="header">
    <a href="/ui/v2">&larr;</a>
    <h2>&#128194; Projects</h2>
    <div class="actions"><button class="new-btn" onclick="location.href='/ui/projects/create'">+ New Project</button></div>
</div>
<div id="projectList"><div style="text-align:center;padding:40px;color:var(--muted);">Loading...</div></div>
<script>
    async function loadProjects(){try{var r=await fetch('/api/projects');var d=await r.json();var ps=d.projects||[];var el=document.getElementById('projectList');if(!ps.length){el.innerHTML='<div class="empty"><div style="font-size:48px;margin-bottom:12px;">&#128194;</div><div>No projects</div><div style="margin-top:8px;"><a href="/ui/projects/create" style="color:var(--accent);">Create first project &rarr;</a></div></div>';return}el.innerHTML='<div class="grid">'+ps.map(function(p){var sc=p.status==='active'?'status-active':'status-draft';return'<a href="/ui/projects/'+p.id+'" class="card"><div class="name">'+(p.name||'Untitled')+' <span class="status '+sc+'">'+(p.status||'draft')+'</span></div><div class="desc">'+(p.description||'').substring(0,80)+'</div><div class="meta"><span>&#128337; '+(p.created_at||'-')+'</span></div></a>'}).join('')+'</div>'}catch(e){el.innerHTML='<div style="text-align:center;padding:40px;color:var(--muted);">Load failed</div>'}}
    loadProjects();
</script>
</body>
</html>
