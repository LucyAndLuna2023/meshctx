...
> 实现: `cluster/cluster_comm_v6.py` · 测试: `tests/test_cluster_comm_v6.py` (57 用例, FakeRedis 离线)
...
## v6.2 组通道 (团队版功能 — 本库仅协议基座)

部门/项目共享信息通道 (hub:group:*) **完整实现在 meshctx-team 私有库**
(协作核心, 团队版 $9)。本库保留: 单播协议 v6.1 (信封校验/去重/journal/
路由白名单) 作为其传输基座; 开源侧 cluster/cluster_groups.py 为 stub
(PEP 562, 访问即抛 TeamFeatureError)。详见 docs/ENTERPRISE_MIGRATION.md。
