...
def test_version_consistency_across_assets():
    """src / src.core / package.json 版本三处一致
    (v3.130.0 起改为相对一致性断言, 避免每次发版改测试; 绝对版本由 G10 门把守)。"""
    import json
    import src
    import src.core as core
    pkg = json.loads((SRC_DIR.parent / "package.json").read_text(encoding="utf-8"))
    assert src.__version__ == core.__version__ == pkg["version"]
