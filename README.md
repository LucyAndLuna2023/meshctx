...
<p align="center">
  ...
  <a href="#"><img src="https://img.shields.io/badge/brain_regions-14-orange"></a>
...
| 场景 | 默认 | 说明 |
|------|------|------|
| 交互式 `meshctx chat` | 6 轮（5 搜 + 1 交付） | `--max-rounds N` 或环境变量 `MESHCTX_MAX_ROUNDS=N` 调整 |
| UI / API | 6 轮 | 请求体 `max_rounds` > 环境变量 `MESHCTX_MAX_ROUNDS` > 默认 6（优先级从高到低） |
| 复杂多步任务 `meshctx agent` | 8 步 | `--max-steps N`，每步最多 3 轮，完成检测自动结束 |

轮次耗尽后会自动交付当前成果，并提示**继续输入以追加轮次**（上下文完整保留）；多轮护栏（web_search ≤ 8 次 / 墙钟 300s / 消息自动裁剪）始终生效，防止失控循环。
...
| v3.115 | **DeepSeek TUI竞品对标** (14模块·4200行) | **130** |
