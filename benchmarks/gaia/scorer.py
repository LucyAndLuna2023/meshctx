...
class GAIAScorer:
    ...
    def score_single(
        ...
        # 判断正确性（2026 规范）
        # L1: exact 或 normalized match
        # L2/L3: exact match 或 F1 ≥ 0.80
        if level == 1:
            result["correct"] = result["exact_match"] or result["normalized_match"]
        else:
            result["correct"] = result["exact_match"] or result["f1_score"] >= 0.80
