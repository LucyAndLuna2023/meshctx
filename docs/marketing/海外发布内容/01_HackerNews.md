...
> **🆕 v3.128.0 更新 (2026-09-07)**: **任意模型手动接入** — 聊天界面直接添加任意模型（供应商 + 模型 ID + API Key + Base URL），key 自动继承同供应商已配密钥。智谱 GLM-4.7/4.7-flash/4.5 系列内置。另：组织审计轨迹升级为防篡改哈希链 + 外部封签。



## 标题（三选一，A 为推荐）

- **A**: `Show HN: MeshCtx – An AI agent with 17 simulated brain regions, with API-controlled parameter evolution`
- **B**: `Show HN: I built an AI agent with a genetic-algorithm engine that tunes its own parameters via API (evolve/feedback)`
...
3. Evolution engine: genetic-algorithm tuning of runtime parameters, triggered via API (evolve/feedback endpoints); auto-feeding from task outcomes is on the roadmap
4. GenomicOptimizer: a 784-line genetic algorithm engine for runtime-parameter tuning (temperature, top_p, prompt style, memory weights) with mutation → crossover → elitism → niche preservation, triggered via API endpoints (evolve/feedback; auto-feeding on roadmap).
...
| "Another AI wrapper?" | 展示 123+ 模型支持 + 智能路由 + 进化引擎（API 受控调参），强调 6 年跨学科积累 |
