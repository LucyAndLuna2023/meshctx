...
# 停止封装版 meshctx 服务（PyInstaller 二进制）
if pgrep -f "meshctx/meshctx" >/dev/null 2>&1; then
    pkill -9 -f "meshctx/meshctx" 2>/dev/null || true
    KILLED=1
fi
...
# 源码/本地模式参数解析（须在 [2/6] 前完成：封装资产优先逻辑依赖 USE_LOCAL）
SOURCE_DIR=""
USE_LOCAL=0
...
# 0) 完整版封装资产优先（护城河 2026-08-23: 终端用户默认安装含闭源核心的完整产品,
#    核心在 PyInstaller 字节码内; 无 Python 的 Mac 也无需安装 Python）
PORTABLE_OK=0
PORTABLE_INSTALL=0
if [ "${USE_LOCAL}" != "1" ]; then
    _ARCH_SUFFIX=""
    case "$(uname -m)" in
        x86_64) _ARCH_SUFFIX="-intel" ;;
    esac
...
if [ -z "${PYTHON_BIN}" ] && [ "${PORTABLE_OK}" != "1" ]; then
    ...
    if command -v brew >/dev/null 2>&1; then
        ...
        if HOMEBREW_NO_AUTO_UPDATE=1 timeout 900 brew install python@3.12 2>/dev/null || HOMEBREW_NO_AUTO_UPDATE=1 timeout 900 brew install python@3.12 2>/dev/null; then
            for p in /usr/local/opt/python@3.12/bin/python3.12 /opt/homebrew/opt/python@3.12/bin/python3.12 \
                     /usr/local/bin/python3.12 /opt/homebrew/bin/python3.12; do
...
# 封装版已就绪 → 跳过 Python PATH/pip/Homebrew 检查
if [ "${PORTABLE_OK}" != "1" ]; then
...
if ! ${PYTHON_BIN} -m pip --version >/dev/null 2>&1; then
    ...
    ${PYTHON_BIN} -m ensurepip --upgrade 2>/dev/null || \
        curl -sS --max-time 60 https://bootstrap.pypa.io/get-pip.py | ${PYTHON_BIN} 2>/dev/null || true
fi
...
if [ "${PORTABLE_OK}" = "1" ]; then
    ...
    # 封装资产已在 [2/6] 尝试下载；失败才走源码 tarball
    if [ "${PORTABLE_OK}" != "1" ]; then
    ...
    DOWNLOAD_OK=0
    if curl -fsSL --connect-timeout 30 --max-time 300 --retry 2 -o "${TARBALL}" "${SRC_URL}" 2>/dev/null; then
        DOWNLOAD_OK=1
...
if [ -d "${INSTALL_DIR}" ]; then
    ...
    # CLI 历史输入 / profiles / 激活标记（重装不得丢失组件, 与 install.sh 对齐）
...
if [ "${PORTABLE_OK}" = "1" ]; then
    ...
    VHOME=$(mktemp -d)
    if ! _PROBE=$(HOME="$VHOME" "${INSTALL_DIR}/meshctx/meshctx" model add deepseek:v4-flash --key sk-gate-verify 2>&1); then
...
if [ -z "$CORE_TOKEN" ] || ! command -v git >/dev/null 2>&1; then
    echo -e "  ${CYAN}→${NC} No MESHCTX_CORE_TOKEN — installing open-source build (full engine, Open Core)."
...
#!/bin/bash
# meshctx CLI wrapper (macOS 封装版)
if [ -f ~/.meshctx/.env ]; then
  set -a; source ~/.meshctx/.env; set +a
fi
exec ~/.meshctx/meshctx/meshctx "$@"
MESHCTX_SCRIPT
else
cat > ~/bin/meshctx << 'MESHCTX_SCRIPT'
#!/bin/bash
...
<dict>
    ...
    <array>
        <string>${INSTALL_DIR}/meshctx/meshctx</string>
        <string>start</string>
        <string>--port</string>
        <string>${PORT}</string>
    </array>
...
if curl -s -o /dev/null -w "%{http_code}" "http://localhost:${PORT}/api/health" 2>/dev/null | grep -q "200"; then
    ...
    # 手动启动
    if [ "${PORTABLE_INSTALL}" = "1" ]; then
        nohup "${INSTALL_DIR}/meshctx/meshctx" start --port ${PORT} > /dev/null 2>&1 &
    else
